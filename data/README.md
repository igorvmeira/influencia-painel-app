...
novo desde 17/08 e 25/08 — o gatilho escrito disparou. Reativadas em 15/09/2026 pela
mesma gravação da `/carteira`, não pela planilha; ver a tabela de exceções.)*
...
| TRAJETO `act_2622092654889646` | **ativa — reativada em 15/09/2026** | 14/09/2026 | **gastou todos os 21 dias de 25/08 a 14/09** — R$ 489,61 em 30 dias | reativada → VINÍCIUS (a aba dela na planilha), por script com prévia e leitura de volta, depois de medir que nenhum selo muda com a régua nova de base incompleta. Mudou na tela: agosto do VINÍCIUS, variação 5,10% → 5,13% |
| DRA. ANA PAULA `act_2057134961683901` | **ativa — reativada em 15/09/2026** | 14/09/2026 | **gastou todos os 29 dias de 17/08 a 14/09** — R$ 829,91 em 30 dias | reativada → ISMAIL (a aba dela na planilha), idem. Mudou na tela: agosto do ISMAIL, CPL R$ 12,68 → R$ 12,75 (−9,26% → −8,74%). Pela régua antiga ela teria tirado o selo de agosto do ISMAIL — era o que travava a reativação |
...
| DRA. ANA PAULA | `act_2057134961683901` | Cadastrada já **pausada** (regra `pausado` = não veicula): último gasto em **02/05/2026**, zero nos últimos 104 dias — *escrito em 14/08/2026*. Entrou pausada para não diluir o CPL de carteira do ISMAIL com uma conta parada. | ✅ **REATIVADA em 15/09/2026** — `pausado: false` e `gestor: "ISMAIL"`, por script com prévia e leitura de volta (decisão do Igor: reativar se nenhum selo mudasse; medido com a régua nova de base incompleta, nenhum mudou). O gatilho tinha disparado em 17/08. ⚠ O `gestorHistorico` registra agora PAUSADO "desde sempre" e ISMAIL desde 15/09: a titularidade ANTERIOR continua escrita só aqui. O gasto de agosto dela (R$ 422,35) entrou no agosto fechado do ISMAIL — CPL R$ 12,68 → R$ 12,75 na tela. |
| TRAJETO | `act_2622092654889646` | Cadastrada e **pausada**. Conta criada em 17/07/2026, `account_status` ACTIVE, mas **gasto zero em 120 dias** — nunca veiculou — *escrito em 06/08/2026*. Ativa, entraria zerada e puxaria o CPL de carteira do gestor para baixo. Está também com **nicho vazio**: a planilha diz "Provedor" e a Meta chama a conta de "CA 01 - TRAJETO MÓVEIS" (*nicho não reconferido em 14/09*). | **Dois gatilhos independentes.** (1) ✅ **REATIVADA em 15/09/2026** — `pausado: false` + `gestor: VINÍCIUS`, idem à DRA. ANA PAULA (o gatilho tinha disparado em 25/08). Os R$ 201,97 de agosto entraram no agosto fechado do VINÍCIUS — variação 5,10% → 5,13% na tela. (2) Preencher o nicho quando a agência disser o ramo — **continua pendente**. |
